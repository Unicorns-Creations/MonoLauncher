	...
	function RamDisplay() {
		...
		return (
			(_ret = ((_temp = ((_this = _possibleConstructorReturn(
				this,
				(_ref = RamDisplay.__proto__ || Object.getPrototypeOf(RamDisplay)).call.apply(
					_ref,
					[ this ].concat(args)
				)
			)),
			_this)),
			(_this.state = { ram: { amount: '0/0GB' } }),
			(_this.intervalID = 0),
			_temp)),
		...
		},
		{
			...
			value: function render() {
				return React.createElement(
					...
					React.createElement('p', null, 'Current Ram Usage: ', this.state.ram.amount)
				);
			}
		}
	]);
...
var DiscordDisplay = (function(_React$Component2) {
	...
	function DiscordDisplay() {
		...
		return (
			(_ret2 = ((_temp2 = ((_this3 = _possibleConstructorReturn(
				this,
				(_ref2 = DiscordDisplay.__proto__ || Object.getPrototypeOf(DiscordDisplay)).call.apply(
					_ref2,
					[ this ].concat(args)
				)
			)),
			_this3)),
			(_this3.state = { discord: { username: 'Username#0000' } }),
			(_this3.intervalID = 0),
			_temp2)),
		...
		},
		{
			...
			value: function render() {
				return React.createElement(
					...
					React.createElement('h3', null, 'Discord User'),
					React.createElement('p', { id: 'discord-holder' }, 'Logged in as: ', this.state.discord.username)
				);
			}
		}
	]);
